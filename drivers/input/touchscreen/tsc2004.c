 ...
 * Revision history:
 *
 * - Basic porting of driver to DTB/DTS support for AM335x MicroSOM.
 *   The GPIO line used for intr detection is statically declared in this module.
 *
 * Author: G. Pavoni Exor  Date: 01/2014
 *
...
/* HACK: Define here the GPIO pin which is used by your platform */
#ifdef CONFIG_SOC_AM33XX /* This is the case of uSOM */
#define	GPIO_TSC2004_IRQ	19
#endif
...
static struct tsc2004_platform_data*
tsc2004_get_devtree_pdata()
{
	...
	pdata->get_pendown_state = tsc2004_get_irq_level;
	pdata->init_platform_hw = tsc2004_init_irq;
	pdata->exit_platform_hw = tsc2004_exit_irq;
//	pdata->irq = gpio_to_irq(GPIO_TSC2004_IRQ);
	return pdata;
}
#endif
				   ...
				   const struct i2c_device_id *id)
{
	...
	#ifdef CONFIG_OF
	pdata = tsc2004_get_devtree_pdata();
	#endif
	...
	#ifdef CONFIG_OF
	ts->irq = gpio_to_irq(GPIO_TSC2004_IRQ);
	#endif
